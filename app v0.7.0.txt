...
def register_korean_font_if_possible() -> str:
    """PDF에서 한글이 깨지지 않도록 폰트를 최대한 자동으로 찾고 등록합니다.

    우선순위:
    1) Windows 기본 폰트(맑은 고딕 등)
    2) Linux(스트림릿 클라우드 등)에서 흔한 한글 폰트(Nanum / Noto CJK)
    3) ReportLab 내장 CID 폰트(UnicodeCIDFont) 시도
    4) 최후: Helvetica (한글은 깨질 수 있음)
    """
    ...
    candidates = [
        ...
        # Linux (Streamlit Cloud / Ubuntu 등)
    ...
    for path in candidates:
        if os.path.exists(path):
            try:
                ...
                # ttc/otf/ttf 등록 실패 가능 → 다음 후보로
                pass

    # ReportLab 내장 CID 폰트(환경에 따라 가능)
    try:
        font_name = "KOR_CID"
        # Korean CID fonts that ReportLab can map on many systems
...
def df_cost_statement(cost_items: dict, cogm: float) -> pd.DataFrame:
    ...
    # ✅ 요약(당기제조원가)
    rows.append(["요약", "당기제조원가(직접재료+직접노무+제조간접)", float(cogm or 0.0), "100.0%" if cogm else ""]) 
    rows.append(["", "", 0.0, ""]) 
...
def build_company_kpi_compare_df(year: int, month: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    """회사별 KPI 비교표(8개 항목).
    - 데이터가 없어도 5개 회사 행을 항상 반환(금액 0)
    - 반환: (원본 numeric df, 표시용 df_view)
    """
    y = int(year)
    m = int(month)

    payload_map = {c: {} for c in COMPANIES}

    packs = load_periods_range_group(y, [m], COMPANIES)
    if packs:
        for company, payload in packs[0].get("_payloads", []):
            if company in payload_map:
                payload_map[company] = payload or {}

    rows = []
    for company in COMPANIES:
        payload = payload_map.get(company, {}) or {}
        p = payload_to_period_input(payload)
        out = compute_all(p)
        ...
        rows.append({
            "회사": company,
            "매출액": float(p.sales),
            "매출원가": float(out.get("cogs", 0.0)),
            "판관비": float(out.get("sga_total", 0.0)),
            "영업이익": float(out.get("op_profit", 0.0)),
            "당기순이익": float(out.get("net_income", 0.0)),
            "공헌이익률": float(out.get("cm_ratio", 0.0)),
            "공헌이익률판정": str(out.get("grade", "")),
            "이자포함BEP": float(out.get("interest_bep_sales", 0.0)),
        })

    df = pd.DataFrame(rows)

    df_view = df.copy()
    money_cols = ["매출액", "매출원가", "판관비", "영업이익", "당기순이익", "이자포함BEP"]
    for c in money_cols:
        df_view[c] = df_view[c].apply(lambda v: f"{float(v or 0.0):,.0f}")
    df_view["공헌이익률"] = df_view["공헌이익률"].apply(lambda v: f"{float(v or 0.0)*100:,.1f}%")
    return df, df_view
...
def build_report_html(title: str, period_label: str, sales_base: float, out: dict,
                      ...
                      tax_mode: str, tax_info: dict | None,
                      kpi_df_view: pd.DataFrame | None = None) -> str:
    tax_note = ""
    if tax_info:
    ...
    kpi_block = ""
    if kpi_df_view is not None and isinstance(kpi_df_view, pd.DataFrame) and (len(kpi_df_view) > 0):
        kpi_block = f"""
        <h3>회사별 KPI 비교표</h3>
        <p><small>※ 동일 기간({period_label}) 기준, 5개 회사를 같은 기준으로 나란히 비교합니다.</small></p>
        {kpi_df_view.to_html(index=False)}
        """
...
def make_pdf_report(title: str, period_label: str, sales_base: float, out: dict,
                    df_pl: pd.DataFrame,
                    df_cost: pd.DataFrame | None,
                    kpi_df_view: pd.DataFrame | None,
                    tax_mode: str, tax_info: dict | None) -> bytes:
    """
    ✅ PDF는 가로(landscape) A4로 출력
    - 손익요약
    - (선택) 제조원가명세서(요약 + 상단 일부)
    - (선택) 회사별 KPI 비교표(8개 항목)
    """
    buf = io.BytesIO()
    ...
    y -= line
    c.drawString(18 * mm, y, f"영업이익: {to_money(out.get('op_profit',0))}   |   공헌이익률: {out.get('cm_ratio',0)*100:,.1f}% ({out.get('grade','')})")
    y -= line
    c.drawString(18 * mm, y, f"이자포함 BEP 매출액: {to_money(out.get('interest_bep_sales',0))}   |   이자포함 BEP 대비 매출: {to_money(out.get('gap_sales_vs_interest_bep',0))}")
    y -= (line * 1.2)
    ...
    if tax_info:
        ...
        c.drawString(18 * mm, y, f"법인세(표시): {note}")
        y -= line
    ...
    col_item_x = 20 * mm
    col_amt_x = width - 70 * mm
    col_ratio_x = width - 18 * mm

    for _, row in df_pl.iterrows():
        item = str(row.get("항목", ""))
        amt = float(row.get("금액", 0.0) or 0.0)
        ratio = str(row.get("매출액 대비 비율", ""))

        c.drawString(col_item_x, y, item)
        c.drawRightString(col_amt_x, y, f"{amt:,.0f}")
        c.drawRightString(col_ratio_x, y, ratio)
    ...
    # 2) 제조원가명세서 (요약 + 상단 일부)
    # =========================
    if df_cost is not None and isinstance(df_cost, pd.DataFrame) and len(df_cost) > 0:
        c.showPage()
        c.setFont(font, 12)
        c.drawString(18 * mm, height - 18 * mm, "제조원가명세서(상세)")
        y = height - 28 * mm
        c.setFont(font, 9)

        # 컬럼 헤더
        headers = ["구분", "항목", "금액", "당기제조원가 대비 비율"]
        col_x = [18*mm, 68*mm, 220*mm, 265*mm]  # landscape 기준 대략 배치
        for hx, h in zip(col_x, headers):
            c.drawString(hx, y, h)
        y -= (line * 0.9)

        # 너무 길어지면 1~2페이지로만 제한 (프린트 가독성)
        max_rows = 55
        show_df = df_cost.head(max_rows).copy()

        for _, r in show_df.iterrows():
            c.drawString(col_x[0], y, str(r.get("구분", ""))[:18])
            c.drawString(col_x[1], y, str(r.get("항목", ""))[:32])
            c.drawRightString(col_x[2], y, f"{float(r.get('금액',0) or 0):,.0f}")
            c.drawRightString(col_x[3], y, str(r.get("당기제조원가 대비 비율", ""))[:10])
        ...
        if len(df_cost) > max_rows:
            c.setFont(font, 9)
            c.drawString(18 * mm, 14 * mm, f"※ 제조원가명세서가 길어 상단 {max_rows}행까지만 표시했습니다. (Excel/HTML에서 전체 확인 가능)")
    ...
    if kpi_df_view is not None and isinstance(kpi_df_view, pd.DataFrame) and len(kpi_df_view) > 0:
        c.showPage()
        c.setFont(font, 12)
        c.drawString(18 * mm, height - 18 * mm, "회사별 KPI 비교표(8개 항목)")
        y = height - 28 * mm
        c.setFont(font, 9)

        # 기대 컬럼(없어도 있는대로 출력)
        cols = list(kpi_df_view.columns)
        # 레이아웃: 회사 + 8개 항목 = 최대 9열
        # 가로폭에 맞춰 균등 분배(회사 열은 조금 넓게)
        left_margin = 18 * mm
        right_margin = 18 * mm
        avail = width - left_margin - right_margin

        company_w = avail * 0.16
        other_w = (avail - company_w) / max(1, (len(cols)-1))
        col_w = [company_w] + [other_w] * (len(cols)-1)
        ...
        # 헤더
        for x, col in zip(col_pos, cols):
            c.drawString(x, y, str(col)[:18])
        y -= (line * 0.95)

        # 데이터
        for _, row in kpi_df_view.iterrows():
            for j, col in enumerate(cols):
                ...
                # 숫자 열 우측 정렬
                if j == 0:
                    c.drawString(x, y, s[:18])
                else:
                    c.drawRightString(x + col_w[j] - 2, y, s[:18])
...
with tab4:
    ...
    # ✅ 회사별 KPI 비교표(8개 항목) - 리포트에 함께 포함
    df_kpi, df_kpi_view = build_company_kpi_compare_df(int(year), int(month))
    ...
    try:
        excel_bytes = make_excel_report(
            ...
            extra_sheets={"회사별 KPI 비교표": df_kpi},
        )
        excel_ok = True
        excel_err = ""
...
# -----------------------------
with tab5:
    st.subheader("회사별 KPI 비교 (선택월 기준)")

    # ✅ 요청사항: 데이터가 없어도 5개 회사 비교표는 항상 표시
    # 표시 항목(8개): 매출액, 매출원가, 판관비, 영업이익, 당기순이익, 공헌이익률, 공헌이익률판정, 이자포함BEP
    selected_year = int(year)
    selected_month = int(month)

    # 월 1개 조회
    pack = load_periods_range_group(selected_year, [selected_month], COMPANIES)
    payload_map = {}
    if pack:
        for c, payload in pack[0].get("_payloads", []):
            payload_map[str(c)] = payload

    rows = []
    for company in COMPANIES:
        payload = payload_map.get(company, {})
        p1 = payload_to_period_input(payload or {})
        out1 = compute_all(p1)

        rows.append({
            "회사": company,
            "매출액": float(p1.sales),
            "매출원가": float(out1.get("cogs", 0.0)),
            "판관비": float(out1.get("sga_total", 0.0)),
            "영업이익": float(out1.get("op_profit", 0.0)),
            "당기순이익": float(out1.get("net_income", 0.0)),
            "공헌이익률": float(out1.get("cm_ratio", 0.0)),
            "공헌이익률판정": grade_cm_ratio(float(out1.get("cm_ratio", 0.0))),
            "이자포함BEP": float(out1.get("interest_bep_sales", 0.0)),
        })
    ...
    for col in money_cols:
        df_view[col] = df_view[col].apply(lambda x: f"{float(x):,.0f}")
    df_view["공헌이익률"] = df_view["공헌이익률"].apply(lambda x: f"{float(x) * 100:,.1f}%")

    st.markdown("### 회사별 KPI 비교표")
